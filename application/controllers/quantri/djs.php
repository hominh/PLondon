<?php
	class Djs extends CI_Controller {
		...
		if ($this->login()) {
				...
				$this->_data ['subview'] = 'backend/dj/index_view';
				$this->_data ['titlePage'] = 'List Dj';
				$this->_data ['info'] = $this->djs_model->getList ();
				$this->_data ['total_new'] = $this->djs_model->countAll ();
		...
		public function delete($id) {
			$this->djs_model->delete($id);
			$this->session->set_flashdata("flash_mess", "Delete Success");
			redirect(base_url() . "quantri/djs");
		}
		
		public function check_name($category) {
			$id=$this->uri->segment(3);
			if ($this->djs_model->checkName($category) == FALSE) {
				$this->form_validation->set_message("check_user", "Dj này đã tồn tại, vui lòng nhập lại");
				return FALSE;
			} else {
				return TRUE;
			}
		...
		public function create() {
			...
			$config['upload_path'] = 'assets/uploads/img_dj/';
			...
			if ($this->login()) {
				...
				$this->_data['subview'] = 'backend/dj/create_view';
				...
				if ($this->form_validation->run() == TRUE) {
					...
					$this->djs_model->insert($data_insert);
		...
		public function edit($id) {
			...
			if ($this->login()) {
				$this->_data['titlePage'] = "Chỉnh sửa bài viết dj";
				$this->_data['subview'] = 'backend/dj/edit_view';
					
				$this->_data['info'] = $this->djs_model->getById($id);
				...
				if ($this->form_validation->run() == TRUE) {
					...
					$this->djs_model->update($data_update, $id);
