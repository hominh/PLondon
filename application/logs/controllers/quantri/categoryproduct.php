 ...
 */ 
    class Categoryproduct extends CI_Controller {
        ...
        public function index() {
            ...
            if ($this->login()) {
                ...
                $this->_data ['subview'] = 'backend/categoryproduct/index_view';
                $this->_data ['titlePage'] = 'List Category Of Product';
                $this->_data ['info'] = $this->categoryproduct_model->getList();
                $this->_data ['total_new'] = $this->categoryproduct_model->countAll();
        ...
        public function create() {
            ...
            $this->_data['categoryproduct'] = $this->categoryproduct_model->getListParent();
            $data['upload_data'] = '';
            ...
            if ($this->login()) { 
                $this->_data['titlePage'] = 'Thêm mới danh mục';
                $this->_data['subview'] = 'backend/categoryproduct/create_view';
                ...
                if ($this->form_validation->run() == TRUE) {
                    ...
                    $data_insert = array(
                        ...
                        "id_parent" => $this->input->post("categoryproduct"),
                        
                       
                        "meta_keyword" => $this->input->post("meta_keyword"),
                        "meta_description" => $this->input->post("meta_description"),
                    );
                    $this->categoryproduct_model->insert($data_insert);
                    $this->session->set_flashdata("flash_mess", "Added");
                    redirect(base_url() . "quantri/categoryproduct");
                }
                $this->load->view('backend/layout/master', $this->_data);
            }
            else {
        ...
        public function check_name($name) {
            ...
            if ($this->categoryproduct_model->checkName($name) == FALSE) {
    ...
    public function edit($id) {
            ...
            $image = base_url() . $config['upload_path'] . $file_name;
            if ($this->login()) {
                $this->_data['titlePage'] = "Chỉnh sửa danh mục sản phẩm";
                $this->_data['subview'] = 'backend/categoryproduct/edit_view';
                $this->_data['info'] = $this->categoryproduct_model->getById($id);
                

                $arr = (object)$this->_data['info'];
                
                $id_parent = $arr->id_parent;
                $this->_data['chuyenmucchabyid'] = $this->categoryproduct_model->getListByParentId($id_parent);
...
$slug_parenttxt = $this->input->post("slug_parenttxt");
                    ...
                    $this->categoryproduct_model->update($data_update,$id);
        ...
        public function delete($id) {
            $this->categoryproduct_model->delete($id);
