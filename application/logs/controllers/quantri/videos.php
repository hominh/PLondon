	...
	class Videos extends CI_Controller {
		...
		public function index() {
			if ($this->login()) {
				...
				$this->_data ['subview'] = 'backend/video/index_view';
				$this->_data ['titlePage'] = 'List Videos';
				$this->_data ['info'] = $this->videos_model->getList ();
				$this->_data ['total_new'] = $this->videos_model->countAll ();
		...
		public function delete($id) {
			...
			redirect(base_url() . "quantri/videos");
		}
		
		public function check_name($category) {
			$id=$this->uri->segment(3);
			if ($this->videos_model->checkName($category) == FALSE) {
				$this->form_validation->set_message("check_user", "Video này đã tồn tại, vui lòng nhập lại");
				return FALSE;
			} else {
				return TRUE;
			}
		...
		public function create() {
			...
			$config['upload_path'] = 'assets/uploads/img_vi/';
			...
			if ($this->login()) {
				...
				$this->_data['subview'] = 'backend/video/create_view';
				...
				if ($this->form_validation->run() == TRUE) {
					...
					$this->videos_model->insert($data_insert);
		...
		public function edit($id) {
			if ($this->login()) { 
				...
				$this->_data['subview'] = 'backend/video/edit_view';
					
				$this->_data['info'] = $this->videos_model->getById($id);
				...
				if ($this->form_validation->run() == TRUE) {
					...
					$this->videos_model->update($data_update, $id);
